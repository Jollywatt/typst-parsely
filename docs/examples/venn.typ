...
This example demonstrates how equations can be turned into anonymous functions which can be evaluated by passing a dictionary defining equation's unknowns.
...
Then, operators are transformed into similar functions which first evaluate their arguments on the given scope and then combine them somehow.
For example, the partially-evaluated node "$#`left` and #`right`$" becomes `scope => left(scope) and right(scope)`.

The final result is a closure which is called with a scope like `("A": true, "B": false)`.
